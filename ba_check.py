...
def main():
    ...
    if all(c in df.columns for c in expected_cols) and 'qty_total_total' in df.columns:
        df['calculated_total'] = df['qty_wheat_total'] + df['qty_fortified_rice_total'] + df['qty_rice_total'] + df['qty_coarse_grains_total']
        mismatches = df[abs(df['calculated_total'] - df['qty_total_total']) > 0.1]
